...
void Manager::gameLoop()
{
	...
	while (bIsRunning)
	{
		...
		while (SDL_PollEvent(&evnt))
		{
			...
			case SDL_MOUSEWHEEL:
				type++;
				if (type > TOTAL_TILE_SPRITES)
					type = 0;
				delete tileSet[0];
				tileSet[0] = new Tile(0, 0, type);
				break;

			case SDL_MOUSEMOTION:
				/*for (int i = 0; i < TOTAL_TILES; i++) {
					SDL_Rect box = tileSet[i]->getBox();
					if ((evnt.button.x > box.x) && (evnt.button.x < box.x + box.w) && (evnt.button.y > box.y) && (evnt.button.y < box.y + box.h)) {
					
						if (type != 0)
						{
							delete tileSet[i];
							tileSet[i] = new Tile(box.x, box.y, type);
						}
					}
				}*/
				break;
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				for (int i = 0; i < TOTAL_TILES; i++) {
					...
					if ((evnt.button.x > box.x) && (evnt.button.x < box.x + box.w) && (evnt.button.y > box.y) && (evnt.button.y < box.y + box.h)) {
						delete tileSet[i];
						const char* c = std::to_string(type).c_str();
						if (type != 0) {
							if (type == TILE_PORTAL) {
								for (int i = 0; i < TOTAL_TILES; i++) {
									if (tileSet[i]->getType() == TILE_PORTAL) {
										delete tileSet[i];
									}
								}
							}
